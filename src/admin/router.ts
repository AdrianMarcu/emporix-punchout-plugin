...
export function createAdminRouter(): Router {
  const router = Router();
  let customerGroupsCache: TokenCache | null = null;
  ...
  router.get('/buyers', wrap(async (req, res) => {
    const tenantId = req.tenantId!;
    const token = extractToken(req);
    const cfg = await getConfig(tenantId, token).catch(() => null);
  ...
  router.post('/buyers', wrap(async (req, res) => {
    const tenantId = req.tenantId!;
    const token = extractToken(req);
    const cfg = await getConfig(tenantId, token).catch(() => null);
    if (!cfg) { res.status(404).json({ error: 'Config not found — save credentials first' }); return; }
  ...
  router.get('/customer-groups', wrap(async (req, res) => {
    const tenantId = req.tenantId!;
    const token = extractToken(req);
    const cfg = await getConfig(tenantId, token).catch(() => null);
    if (!cfg) { res.json([]); return; }
    if (!customerGroupsCache || customerGroupsCacheClientId !== cfg.serviceAccount.clientId) {
      customerGroupsCache = new TokenCache(
        appConfig.emporix.apiBase, tenantId,
        cfg.serviceAccount.clientId, cfg.serviceAccount.clientSecret,
      );
  ...
  // Global error handler for this router — always includes CORS-safe JSON
  router.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[admin] unhandled error:', msg);
